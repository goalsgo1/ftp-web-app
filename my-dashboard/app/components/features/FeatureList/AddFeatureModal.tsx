...
export default function AddFeatureModal({ isOpen, onClose, onSuccess }: AddFeatureModalProps) {
  const [formData, setFormData] = useState({
    ...
    newsKeywords: [] as string[], // 뉴스 스크래퍼 키워드 (뉴스 스크래퍼 선택 시만)
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      if (isNewsScraper) {
        // 키워드 저장 (없으면 빈 배열)
        featureData.newsKeywords = formData.newsKeywords.length > 0 ? formData.newsKeywords : [];
        featureData.newsSources = formData.newsSources;
      }
      
      await addFeature(featureData, user?.uid);
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          {isNewsScraper && (
            <>
              {/* 뉴스 키워드 입력 */}
              <div>
                <label htmlFor="newsKeywords" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                  <FiTag className="inline w-4 h-4 mr-1" />
                  수집할 뉴스 키워드 (선택사항)
                </label>
              ...
              {/* 뉴스 소스 선택 */}
              <div>
                <label htmlFor="newsSources" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
